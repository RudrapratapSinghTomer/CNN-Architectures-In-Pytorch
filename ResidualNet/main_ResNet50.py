...
class BottleNeck(nn.Module):
    def __init__(self, in_channel, out_channel, downsample, stride, expansion: int = 4):
        ...
        final_out_channels = out_channel * expansion

        self.conv3 = nn.Conv2d(out_channel, out_channel * expansion, kernel_size=1, bias=False)
        self.bnorm3 = nn.BatchNorm2d(out_channel * expansion)
...
class ResNet_50(nn.Module):
    def __init__(self, img_channel, num_layer = 50, num_channel = 10):
        ...
        self.conv3 = self.makelayer(BottleNeck, 256, layers[2], stride=2)
        self.conv4 = self.makelayer(BottleNeck, 512, layers[3], stride=2)
    ...
    def makelayer(self, block: type[BottleNeck], out_channels, blocks, stride=2):
        downsample = None
        if stride != 1 or self.in_channnel != out_channels * self.expansion:
            downsample = nn.Sequential(nn.Conv2d(in_channels=self.in_channnel, out_channels=out_channels*self.expansion, kernel_size=1, stride=stride),
                                            nn.BatchNorm2d(out_channels*self.expansion),
                                            )
        
        layers = []
        layers.append(BottleNeck(in_channel=self.in_channnel ,out_channel=out_channels, stride=stride, expansion=self.expansion, downsample=downsample))
...
model = ResNet_50(img_channel=3).to(device)
